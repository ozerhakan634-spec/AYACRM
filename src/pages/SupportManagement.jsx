...
const SupportManagement = () => {
  ...
  const loadTickets = async () => {
    try {
      ...
      if (result.success) {
        ...
        if (toast) toast.error('Destek talepleri yüklenemedi');
      }
    } catch (error) {
  ...
  const handleStatusChange = async (ticketId, newStatus) => {
    try {
      const result = await SupportService.updateSupportTicket(ticketId, { status: newStatus });
      
      if (result.success) {
        if (toast) toast.success('Durum başarıyla güncellendi');
        await loadTickets(); // Listeyi yenile
      } else {
        if (toast) toast.error(`Durum güncellenirken hata oluştu: ${result.error}`);
      }
    } catch (error) {
  ...
  const handleResponseSubmit = async () => {
    ...
    try {
      ...
      const result = await SupportService.updateSupportTicket(selectedTicket.id, {
      ...
      if (result.success) {
        ...
        if (toast) toast.error(`Yanıt gönderilirken hata oluştu: ${result.error}`);
      }
    } catch (error) {
  ...
  const handleDeleteTicket = async (ticketId) => {
    ...
    try {
      const result = await SupportService.deleteSupportTicket(ticketId);
      
      if (result.success) {
        if (toast) toast.success('Destek talebi başarıyla silindi');
        await loadTickets(); // Listeyi yenile
      } else {
        if (toast) toast.error(`Silme hatası: ${result.error}`);
      }
    } catch (error) {
