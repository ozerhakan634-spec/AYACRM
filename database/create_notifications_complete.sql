-- Bildirim Sistemi Tam Kurulum
-- Bu dosyayı Supabase SQL Editor'da çalıştırın

-- 1. Notifications tablosunu oluştur (eğer yoksa)
...
BEGIN
  ...
  IF NEW.consultant_id IS NOT NULL AND (OLD.consultant_id IS NULL OR NEW.consultant_id != OLD.consultant_id) THEN
    INSERT INTO notifications (user_id, title, message, type, category, related_id, related_type)
    VALUES (
      NEW.consultant_id,
      'Yeni Müşteri Atandı',
      'Size yeni bir müşteri atandı: ' || COALESCE(NEW.name, 'Müşteri'),
      'success',
      'client',
      NEW.id,
      'client'
...
BEGIN
  ...
  IF NEW.assigned_to IS NOT NULL AND (OLD.assigned_to IS NULL OR NEW.assigned_to != OLD.assigned_to) THEN
    ...
    VALUES (
      NEW.assigned_to,
      'Yeni Görev Atandı',
      'Size yeni bir görev atandı: ' || COALESCE(NEW.title, 'Görev'),
      'info',
      'task',
      NEW.id,
      'task'
...
-- 8. Mevcut müşterileri kontrol et
SELECT 
  id,
  name,
  consultant_id,
